...
npm run db:bundle              # dist/all-migrations.sql for SQL Editor (001–013)
...
- **Next:** Apply prod migrations `001`–`013` (`npm run db:push` / `db:bundle`). `npm run kb:sync`. Live Stripe (`scripts/STRIPE_SETUP.md`). Deploy (`scripts/DEPLOY.md`). MQTT/HA/Tuya → hardware runtime after hub transport. Hardware Pass 3 deferred (hub pending).
